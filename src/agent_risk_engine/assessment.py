...
class RiskEvaluator:
    ...
    async def evaluate(self, tool_name: str, args: dict, tool_risk: int) -> RiskResult:
        ...
        # Layer 3: Check system state
        system_state = self.state_monitor.check()

        # Layer 4: Final decision
        final = self.action_gate.decide(rule_result, risk_score, system_state)
